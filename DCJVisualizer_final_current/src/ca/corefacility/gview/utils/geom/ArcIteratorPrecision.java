 ...
 * Allows for higher precision of segments than default ArcIterator.
 *
 * @author      Jim Graham
 * 
 * Modified by Aaron Petkau
 */
class ArcIteratorPrecision implements PathIterator {
    ...
    ArcIteratorPrecision(int minSegments, Arc2DPrecision a, AffineTransform at) {
        this.w = a.getWidth() / 2;
        ...
        if (ext >= 360.0 || ext <= -360) {
            arcSegs = minSegments;
            this.increment = (2*Math.PI)/minSegments;
        	...
        	int minSegSection = (int)(minSegments*(Math.abs(ext)/360.0))+1;
            arcSegs = (int) Math.ceil(Math.abs(ext) / 90.0);
            arcSegs = (arcSegs < minSegSection) ? minSegSection : arcSegs;
        ...
        switch (a.getArcType()) {
        case Arc2DPrecision.OPEN:
            lineSegs = 0;
            break;
        case Arc2DPrecision.CHORD:
            lineSegs = 1;
            break;
        case Arc2DPrecision.PIE:
            lineSegs = 2;
            break;
        }
        if (w < 0 || h < 0) {
